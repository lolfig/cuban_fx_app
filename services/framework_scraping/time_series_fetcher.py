...
class PriceTimeSeries:
    ...
    async def sync_time_series(self):
        ...
        for offer in self.offers:
            ...
            if data_toque is not None:
                ...
                self.toque_serie = (
                    pd.merge(self.toque_serie, merged_df, on="date", how="outer")
                    if not self.toque_serie.empty
                    else merged_df
                )
    ...
    def _process_offer_data(self, data_toque: List[dict], offer: str) -> pd.DataFrame:
        ...
        date_range_df = pd.DataFrame(
            {
                "date": pd.date_range(
                    start=serie["date"].min(), end=serie["date"].max(), freq="D"
                )
            }
        )

        merged_df = (
            date_range_df.merge(serie, on="date", how="left")
    ...
    def save_data_info(self):
        with open(os.path.join(self.path, f"{self.currency}CUP.pickle"), "wb") as file:
            pickle.dump(self, file)  # noqa este file es writeable

    @staticmethod
    def load_from_file(path, currency: CURRENCIES) -> "PriceTimeSeries":
        with open(os.path.join(path, f"{currency}CUP.pickle"), "rb") as file:

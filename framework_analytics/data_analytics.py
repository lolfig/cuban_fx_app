...
from framework_analytics.analytics_utils import find_marginal_price
...
class DataAnalytics:
    ...
    def _do_intra_day_analytics(self, df, date):
        ...
        """
        compra_df, venta_df = self._extract_orders(df)

        # Calculate volume and price levels
        nivel_volumen_compra, nivel_precio_compra = self._calculate_levels(compra_df)
        nivel_volumen_venta, nivel_precio_venta = self._calculate_levels(venta_df, is_sell=True)

        price, vol = find_marginal_price(
            nivel_precio_compra, nivel_volumen_compra, nivel_precio_venta, nivel_volumen_venta
        )
    ...
    def _extract_orders(self, df):
        ...
        for order in df['orders']:
            if order:
                if order['sign'] == 'compro':
                    buy_orders.append({'vol_compra': order['size'], 'precio_compra': order['price']})
                elif order['sign'] == 'vendo':
                    sell_orders.append({'vol_venta': order['size'], 'precio_venta': order['price']})

        return pd.DataFrame(buy_orders), pd.DataFrame(sell_orders)
    ...
    def _calculate_levels(self, orders_df, is_sell=False):
        ...
        volumen_nivel = np.array(orders_df['vol_compra' if not is_sell else 'vol_venta'].cumsum())

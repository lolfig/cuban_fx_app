...
fully_space = [
    ...
    (
        statistics_graph_row := dbc.Row(
            [
                dbc.Col(
                    graph_histogram_vol_price := dcc.Graph(id="histogram-vol-price"),
                    width=6,
                ),
    ...
    dbc.Row(
        [
            dbc.Col(
                width=12,
                children=(slot_messages_table := html.Div(id="messages-table")),
                style={
    ...
    my_components.head_lines.row_h3("Serie Temporal: Número de Mensajes"),
    dbc.Row(
        [
    ...
    my_components.head_lines.row_h3("Modelo Oculto de Markov"),
    slot_hidden_markov_model := dbc.Row([]),
    my_components.head_lines.row_h3("Descomposición de Modos Empíricos"),
    slot_empirical_mode_decomposition := dbc.Row([]),
]
...
layout = dbc.Container(
    [
        ...
        main_dashboard_container := dbc.Container(children=[empty_space]),
    ]
)
